...
class StreamingConfig:
    ...
    DEFAULT_CANCELLATION_CHECK_INTERVAL = getattr(
        grpc_settings, "STREAMING_CANCELLATION_CHECK_INTERVAL", 100
    )
...
def check_context_cancelled(context: ServicerContext | AsyncServicerContext) -> bool:
    """
...
def stream_queryset_sync(
    ...
    cancellation_check_interval: int | None = None,
...
async def stream_queryset_async(
    ...
    serializer_class,
    serializer_kwargs: dict | None = None,
    context: AsyncServicerContext | None = None,
    chunk_size: int | None = None,
    cancellation_check_interval: int | None = None,
    ...
    try:
        ...
        async for item in async_iterator():
            ...
            def serialize_item(current_item):
                serializer = serializer_class(current_item, **serializer_kwargs)
                return serializer.message

            message = await sync_to_async(serialize_item, thread_sensitive=True)(item)
            ...
            try:
                ...
                logger.warning(
                    f"Yield timeout after {yield_timeout}s, client may be disconnected"
                )
                break
            except Exception as e:
